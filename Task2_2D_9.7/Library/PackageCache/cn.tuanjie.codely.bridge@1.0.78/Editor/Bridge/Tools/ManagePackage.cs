    ...
    public static class ManagePackage
    {
        ...
        private static readonly Dictionary<string, Func<JObject, object>> ActionHandlers =
            new Dictionary<string, Func<JObject, object>>
            {
                ...
                { "wait_for_upm", WaitActionDeprecated },
        ...
        private static object WaitActionDeprecated(JObject @params)
        {
            ...
            return Response.ErrorWithCode(
                "action_deprecated",
                $"Action '{action}' is deprecated. There is no need to wait: install_package and " +
                "remove_package return only after the operation is done. To wait for the editor " +
                "to settle after external work, use 'unity_editor { \"action\": \"wait_for_idle\" }'.");
        }
